...
void user_main(void* arg) {
    ...
    status = task_destroy(agrup);
    assert(status == NOERROR);
    status = task_destroy(cons[0]);
    assert(status == NOERROR);
    status = task_destroy(cons[1]);
    assert(status == NOERROR);
    status = task_destroy(prod[0]);
    assert(status == NOERROR);
    status = task_destroy(prod[1]);
    assert(status == NOERROR);
    status = task_destroy(prod[2]);
